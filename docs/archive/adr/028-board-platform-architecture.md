...
Froth needs to run on multiple hardware targets (ESP32, RP2040, bare-metal ARM, POSIX host) while keeping the kernel portable and making it easy for newcomers to add support for new boards. The first non-POSIX target is ESP32 DevKit V1 (original ESP32, dual-core Xtensa, ESP-IDF).
...
- Must produce structured metadata for editor tooling and repeatable board scaffolding
...
Trade-offs: more upfront structure, but each layer has a clear audience and rate of change. Declarative board definitions enable tooling and repeatable scaffolding. Slightly more complex build system.
...
One per physical board or hardware configuration. The thing newcomers write, and the editor can eventually understand.
...
- The small structured schema makes it practical to scaffold `board.json` + `lib.froth` from a datasheet with normal tooling.
