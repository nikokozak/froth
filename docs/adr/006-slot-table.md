...
typedef struct {
  ...
  froth_cell_t impl;       // QuoteRef for Froth-defined words
  froth_primitive_fn_t prim; // non-NULL for primitives, NULL for Froth-defined
} froth_slot_t;
```
...
- **`impl`**: a tagged cell. For Froth-defined words, this is a QuoteRef whose payload indexes into the heap where the quotation body lives. For primitives, this field is unused (initialized to 0).
- **`prim`**: a C function pointer typedef'd as `froth_primitive_fn_t`. If non-NULL, `call` invokes this directly instead of interpreting `impl`. The signature `froth_error_t (*)(void)` gives primitives full access to the global stacks; returning a non-OK error triggers the error recovery path.
...
3. Otherwise, `impl` must be a QuoteRef — execute its body.
...
- Primitives and Froth-defined words coexist in the same table, distinguished by `prim != NULL`.
...
- Spec Section 5.2: `def` binds a QuoteRef to a slot
