...
| CLI | `tools/cli` is a real shipped product surface; checkout builds emit `froth-cli`, release assets ship `froth`, and the Go module/import path now uses Frothy-owned identity internally. | `README.md`, `tools/cli/go.mod`, `.github/workflows/release.yml` | `Go`, minimal `Shell` | Packaged and local | `keep` |
...
| Identity | `tools/cli/go.mod` and Go imports now use `github.com/nikokozak/frothy/...` | Buildable CLI module path with Frothy-owned identity | Frothy ADR-109, `tools/cli/go.mod`, `tools/cli/main.go` | Module declaration and imports now match the product repo identity while leaving the installed binary name unchanged | `Go` | Packaged and local | `keep` | Keep any future installed-binary rename separate from the now-landed module-path cleanup | current installed CLI binary remains `froth` | `cd tools/cli && go test ./... && go list ./...` |
