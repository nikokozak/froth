...
| Release automation | CLI tarballs, VSIX packaging, firmware zips, Homebrew metadata, and release workflows are all active; release packaging now keeps its dependency budget to `Go`, `Shell`, and `Node` only. | `.github/workflows/release.yml`, `tools/package-release.sh`, `tools/package-firmware-release.sh`, `tools/release-common.sh` | `Go`, `Shell`, `Node` | Release-only | `merge` |
...
| Packaging | CLI tarball packaging, firmware zip packaging, VSIX packaging, and release notes | Ship all released artifacts | `.github/workflows/release.yml`, `tools/package-release.sh`, `tools/package-firmware-release.sh`, `tools/vscode/package.json` | Packaging is functionally complete and now uses a smaller runtime budget: `Go` + `Shell` for CLI/firmware and `Node` only for the extension | `Go`, `Shell`, `Node` | Release-only | `merge` | Keep the release surface small and keep Node scoped to the extension only | none on the release-packaging side; hardware proofs still keep the separate Python exception | `.github/workflows/release.yml` succeeds on all three artifact jobs |
