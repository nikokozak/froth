...
| Proof harness | The maintained proof story is now centered on root `make`, the Go test runner, CTest, and thin shell wrappers; Python remains quarantined to explicit hardware lanes only. | Frothy ADR-109, `Makefile`, `tools/frothy/proof.sh`, `tools/cli/cmd/test-runner/*` | `C`, `Go`, `Shell`, temporary `Python` on hardware lanes | Local and CI | `merge` |
...
| Reference and archive docs | `docs/archive/` and `docs/reference/` are legitimate reference surfaces, not product surfaces. They should stay explicit and non-authoritative. | `the repo guide`, `docs/spec/README.md`, `docs/adr/README.md`, `docs/reference/Froth_Substrate_References.md` | none | Local-only | `archive` |
...
| Proofs | Proof orchestration is split across `make`, Go, shell, and Python | Run host tests, CLI tests, extension-local proofs, and hardware smokes | Frothy ADR-109, `Makefile`, `tools/frothy/proof.sh`, `tools/cli/cmd/test-runner/proofs.go` | Root `make` builds a Go test runner; `make test` and `make test-all` stay on the core `C`/`Go`/`Shell` lanes, while `make test-publishability` adds the shipped VS Code host smoke without pretending the real-device editor lane is automatic | `C`, `Go`, `Shell`, temporary `Python`, extension-local `Node` | Local and CI | `merge` | Keep one proof matrix, one explicit full-surface local gate, and thin shell wrappers only where they add no policy | ESP32 device lanes still need distinct device handling | `make test && make test-all && make test-publishability && make test-list` |
...
| Proofs | Root proof entrypoints, Go runner, shell wrappers, and hardware lanes | Run local, CI, and hardware verification | Frothy ADR-109, `Makefile`, `tools/frothy/proof.sh`, `tools/cli/cmd/test-runner/*` | There are too many entrypoints for what is conceptually one proof system | `C`, `Go`, `Shell`, temporary `Python` | Local and CI | `merge` | Keep one proof matrix and one main dispatcher, with `make test-publishability` as the explicit full local shipped-surface gate; demote shell to thin wrappers and quarantine hardware-only exceptions | hardware lanes still need distinct device handling | `make test && make test-all && make test-publishability && make test-list` |
| Packaging | CLI tarball packaging, workshop export verification, VSIX packaging, and release notes | Ship the released public artifacts only | Frothy ADR-122, `.github/workflows/release.yml`, `tools/package-release.sh`, `tools/vscode/package.json`, `tools/frothy/export_workshop_repo.sh` | Packaging is functionally complete and now uses a smaller runtime budget: `Go` + `Shell` for CLI and workshop export, and `Node` only for the extension | `Go`, `Shell`, `Node` | Release-only | `merge` | Keep the release surface small and keep Node scoped to the extension only | none on the public release-packaging side; hardware proofs still keep the separate Python exception | `.github/workflows/release.yml` succeeds on the CLI, workshop-surface, and VSIX jobs |
...
| Dependency budget | Node and Python exceptions | Support the extension and temporary hardware/release helpers | Frothy ADR-111, `tools/vscode/package.json`, `tools/frothy/proof_m10_smoke.sh`, `tools/package-firmware-release.sh` | Node is justified by the kept VSIX surface and now sits behind `make test-vscode` / `make test-publishability`; Python is only justified by temporary hardware/release helpers | `Node`, temporary `Python` | Release-only and hardware-only | `quarantine` | Make exceptions explicit, prevent them from entering the core root proof path, and remove Python from release first | hardware smokes still use Python today | `make test && make test-all` with no Node or Python required, plus `make test-publishability` when the shipped extension surface changes |
