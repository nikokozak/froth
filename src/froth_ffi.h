 ...
 * Usage:  FROTH_FFI(gpio_write, "gpio.write", "( pin val -- )", "Write digital output") { ... }
 * The VM pointer is always named `froth_vm`, which FROTH_POP/FROTH_PUSH depend on. */
#define FROTH_FFI(fn_name, name_str, effect, help_str)          \
  ...
  static const froth_ffi_entry_t fn_name##_entry = {             \
    .name = name_str, .word = fn_name,                           \
    .stack_effect = effect, .help = help_str,                    \
  };                                                             \
  static froth_error_t fn_name(froth_vm_t *froth_vm)

/* Pop a number into a new local variable. Requires froth_vm in scope. */
#define FROTH_POP(into)  froth_cell_t into; FROTH_TRY(froth_pop(froth_vm, &into))

/* Push a number onto DS. Requires froth_vm in scope. */
#define FROTH_PUSH(value) FROTH_TRY(froth_push(froth_vm, (value)))

/* Reference the metadata struct created by FROTH_FFI for use in a binding table. */
