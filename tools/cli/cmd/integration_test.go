...
func TestIntegrationNewBuildAndRunAutorun(t *testing.T) {
	...
	binaryPath := filepath.Join(projectDir, ".froth-build", "firmware", "Froth")
	output := runBinaryAfterBoot(t, binaryPath, projectDir, "", 20*time.Second)
...
`)
	mustWriteIntegrationFile(t, filepath.Join(projectDir, "lib", "helpers.froth"), `: lib-word "LIB" ;
`)
	mustWriteIntegrationFile(t, filepath.Join(projectDir, "src", "extra.froth"), `: extra-word "EXTRA" ;
`)
...
`)
	...
	binaryPath := filepath.Join(projectDir, ".froth-build", "firmware", "Froth")
	output := runBinaryAfterBoot(t, binaryPath, projectDir, "", 20*time.Second)
	assertContains(t, output, "LIB EXTRA")
}
...
func TestIntegrationConnectLocalBuildsAndLaunchesBinary(t *testing.T) {
	...
	output := runProcessAfterBoot(t, workspace, env, 3*time.Minute, "", integrationCLIPath, "connect", "--local")
	assertContains(t, output, "boot: CTRL-C for safe boot")
	assertContains(t, output, "froth>")

	home := integrationHomeFromEnv(t, env)
	localBinary := filepath.Join(home, "local-build", "Froth")
...
func runBinaryAfterBoot(t *testing.T, binaryPath string, dir string, source string, timeout time.Duration) string {
	t.Helper()
	return runProcessAfterBoot(t, dir, os.Environ(), timeout, source, binaryPath)
}
...
func runProcessAfterBoot(t *testing.T, dir string, env []string, timeout time.Duration, source string, command string, args ...string) string {
	...
	buf := &watchedBuffer{
		needle: "boot: CTRL-C for safe boot",
	...
	case <-ctx.Done():
		_ = stdin.Close()
		_ = cmd.Wait()
		t.Fatalf("waiting for boot banner: %v\n%s", ctx.Err(), buf.String())
	}
